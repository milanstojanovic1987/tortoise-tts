...
def pad_or_truncate(t, length):
    """
    Utility function for forcing <t> to have the specified sequence length,
    whether by clipping it or padding it with 0s.
    """
    if t.shape[-1] == length:
        return t
    elif t.shape[-1] < length:
...
def fix_autoregressive_output(codes, stop_token, complain=True):
    """
    This function performs some padding on coded audio that fixes a mismatch issue
    between what the diffusion model was trained on and what the autoregressive code generator creates.
    """
    stop_token_indices = (codes == stop_token).nonzero()
    if len(stop_token_indices) == 0:
        if complain:
...
def do_spectrogram_diffusion(diffusion_model, diffuser, latents, conditioning_latents, temperature=1, verbose=True):
    ...
    with torch.no_grad():
        ...
        mel = diffuser.p_sample_loop(
            diffusion_model, output_shape, noise=noise,
            model_kwargs={'precomputed_aligned_embeddings': precomputed_embeddings},
            progress=verbose
        )
        return denormalize_tacotron_mel(mel)[:, :, :output_seq_len]
...
def classify_audio_clip(clip):
    ...
    classifier = AudioMiniEncoderWithClassifierHead(
        2, spec_dim=1, embedding_dim=512, depth=5, downsample_factor=4,
        resnet_blocks=2, attn_blocks=4, num_attn_heads=4, base_channels=32,
        dropout=0, kernel_size=5, distribute_zero_label=False
    )
    classifier.load_state_dict(torch.load(get_model_path('classifier.pth'), map_location=torch.device('cpu')))
...
def pick_best_batch_size_for_gpu():
    """
    Tries to pick a batch size that will fit in your GPU. 
    These sizes aren't guaranteed to work, but should give you a good shot.
    """
    if torch.cuda.is_available():
        _, available = torch.cuda.mem_get_info()
        availableGb = available / (1024 ** 3)
...
class TextToSpeech:
    ...
    def __init__(
        self, autoregressive_batch_size=None, models_dir=MODELS_DIR,
        enable_redaction=True, kv_cache=False, use_deepspeed=False, half=False, device=None,
        tokenizer_vocab_file=None, tokenizer_basic=False
    ):
        """
        ...
        if os.path.exists(f'{models_dir}/autoregressive.ptt'):
            ...
            self.diffusion = DiffusionTts(
                model_channels=1024, num_layers=10, in_channels=100, out_channels=200,
        ...
        self.clvp = CLVP(
            dim_text=768, dim_speech=768, dim_latent=768, num_text_tokens=256,
            text_enc_depth=20, text_seq_len=350, text_heads=12,
            num_speech_tokens=8192, speech_enc_depth=20, speech_heads=12, speech_seq_len=430,
            use_xformers=True
        ).cpu().eval()
        self.clvp.load_state_dict(torch.load(get_model_path('clvp2.pth', models_dir)))
        ...
        self.intermediate_mels = []  # ADDED: will store intermediate mel spectrograms
    ...
    def load_cvvp(self):
        """Load CVVP model."""
        self.cvvp = CVVP(
            model_dim=512, transformer_heads=8, dropout=0, mel_codes=8192,
            conditioning_enc_depth=8, cond_mask_percentage=0,
            speech_enc_depth=8, speech_mask_percentage=0, latent_multiplier=1
        ).cpu().eval()
        self.cvvp.load_state_dict(torch.load(get_model_path('cvvp.pth', self.models_dir)))
        ...
        with torch.no_grad():
            ...
            for sample in voice_samples:
                ...
                cond_mel = wav_to_univnet_mel(
                    sample.to(self.device), do_normalization=False,
                    device=self.device, stft=self.stft
                )
                diffusion_conds.append(cond_mel)
            diffusion_conds = torch.stack(diffusion_conds, dim=1)
    ...
    def tts_with_preset(self, text, preset='fast', **kwargs):
        ...
        """
        settings = {
            'temperature': .8, 'length_penalty': 1.0, 'repetition_penalty': 2.0,
        ...
        settings.update(presets[preset])
        settings.update(kwargs)  # allow overriding preset settings
        return self.tts(text, **settings)

    def tts(
        self, text, voice_samples=None, conditioning_latents=None, k=1, verbose=True,
        use_deterministic_seed=None, return_deterministic_state=False,
        num_autoregressive_samples=512, temperature=.8, length_penalty=1,
        repetition_penalty=2.0, top_p=.8, max_mel_tokens=500,
        cvvp_amount=.0,
        diffusion_iterations=100, cond_free=True, cond_free_k=2, diffusion_temperature=1.0,
        **hf_generate_kwargs
    ):
        ...
        # Reset intermediate_mels at each new run (optional, if you want a clean list each time)
        self.intermediate_mels = []  # ADDED: Clear any previous stored mels

        text_tokens = torch.IntTensor(self.tokenizer.encode(text)).unsqueeze(0).to(self.device)
        text_tokens = F.pad(text_tokens, (0, 1))
        assert text_tokens.shape[-1] < 400, 'Too much text provided.'
        ...
        with torch.no_grad():
                ...
                with self.temporary_cuda(self.autoregressive) as autoregressive, torch.autocast(
                    ...
                    for b in tqdm(range(num_batches), disable=not verbose):
                        codes = autoregressive.inference_speech(
                            auto_conditioning, text_tokens, do_sample=True,
                ...
                with self.temporary_cuda(self.clvp) as clvp, torch.autocast(
                    ...
                    if verbose:
                        if self.cvvp is None:
                            ...
                            print(f"Computing best candidates using CLVP {((1-cvvp_amount) * 100):2.0f}% "
                                  f"and CVVP {(cvvp_amount * 100):2.0f}%")
                    for batch in tqdm(samples, disable=not verbose):
                        for i in range(batch.shape[0]):
                            batch[i] = fix_autoregressive_output(batch[i], stop_mel_token)
                        if cvvp_amount != 1:
            ...
            # The diffusion model wants the last hidden layer from the autoregressive model as conditioning
            if not torch.backends.mps.is_available():
                with self.temporary_cuda(self.autoregressive) as autoregressive, torch.autocast(
                    device_type="cuda", dtype=torch.float16, enabled=self.half
                ):
                    best_latents = autoregressive(
                        ...
                        best_results,
                        torch.tensor([best_results.shape[-1] * self.autoregressive.mel_length_compression],
            ...
            if not torch.backends.mps.is_available():
                with self.temporary_cuda(self.diffusion) as diffusion, self.temporary_cuda(self.vocoder) as vocoder:
                    for b in range(best_results.shape[0]):
                        ...
                        # Find the first occurrence of the "calm" token and trim the codes
                        ctokens = 0
                        for idx in range(codes.shape[-1]):
                            if codes[0, idx] == calm_token:
                                ctokens += 1
                            else:
                                ctokens = 0
                            if ctokens > 8:
                        ...
                        mel = do_spectrogram_diffusion(
                            diffusion, diffuser, latents, diffusion_conditioning,
                            temperature=diffusion_temperature, verbose=verbose
                        )

                        # ADDED: store intermediate mel
                        self.intermediate_mels.append(mel.detach().cpu())
